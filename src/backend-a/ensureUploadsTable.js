...
// Ensure the `uploads` and `requests` tables exist in PostgreSQL for Backend A.
 ...
 */
async function ensureUploadsTable(backendNameHint = 'backend-a') {
  ...
  try {
    ...
    const patchRequestsSchemaSql = `
      DO $$
      BEGIN
        -- Add missing columns if table existed with a different schema
        IF NOT EXISTS (
        ...
        -- Backfill ts safely (use created_at if it exists)
        IF EXISTS (
          SELECT 1 FROM information_schema.columns
